...
def depth2screen(depth_depth_val, depth_x_val):
    # tx = -1489115000 + (1880.618 - -1489115000) / (1 + (depth_x_val / 72240570) ** 1.16871)
    tx = 1881.345 - 2.136646 * depth_x_val - 0.00126057 * depth_x_val ** 2
    ty = 856.6725 + (-1247.19 - 856.6725) / (1 + (depth_depth_val / 221.7208) ** 46.88866)
...
while True:
    ...
    if not ret:
        break
    depth_roi = 255 - depth[210:, 60:]
    depth_roi_canvas = depth_roi.copy()

    _, mask = cv2.threshold(depth_roi, 220, 255, cv2.THRESH_BINARY)
    ...
    if len(contours) > 0:
        ...
        closest_point = min(contour, key=lambda l: l[0][1])[0]
        cv2.circle(depth_roi_canvas, (closest_point[0], closest_point[1]), 10, 0, -1)
        if closest_point[1] == 0:
            if not did_click:
                did_click = True
                click_pos = depth2screen(depth_roi[closest_point[1], closest_point[0]], closest_point[0])
        else:
            did_click = False
    ...
    if start_menu:
        ...
        # cv2.imshow('Depth', depth_roi_canvas)
        key = cv2.waitKey(1)
        if key == ord('q'):
            break
        fps.increment()
